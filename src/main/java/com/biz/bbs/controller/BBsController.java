...
public class BBsController {
	...
	public String write(@ModelAttribute("bbsReqDto") BBsReqDto bbsReqDto ,Model model
	...
	public String write_up(@ModelAttribute("bbsReqDto") @Valid BBsReqDto bbsReqDto,
		 ...
		 * vo(Dto)에 한번 담긴 값은 웹브라우져가 열려있는 동안
		 * 항상 유지되는 성질이 있다
		 * 
		 * write(POST) , update(POST)를 수행한 후에는 
	...
	/*
	 *  PathVariable
	 *  	GET 방식으로 서버에 데이터를 보낼 때
	 *  	?변수 = 값 형식으로 많이 사용을 한다.
	 *  	그런데 이 방식에 보안이 취약점이 될 수 있다
	 *  
	 *  	path/값 주소처럼 서버로 데이터를 보내고 
	 *  	controller에서는 주소의 일부를 값으로 인식하여
	 *  	변수에 할당하는 방식
	 */
	...
	public String reply(@PathVariable long bbs_seq,
		...
		//답글달기에서는 bbs_main_seq에 원글의 bbs_seq 값을 포함해야한다
	...
	public String reply_write(@PathVariable long bbs_seq, 
						@ModelAttribute("bbsReqDto") BBsReqDto bbsReqDto, 
						SessionStatus sStatus ,Model model) {
